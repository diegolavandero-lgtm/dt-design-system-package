...
(function () {
  ...
  const DICT = {
    ...
    "nav.mobile.bottomSheets": { "es": "Bottom sheets", "en": "Bottom sheets" },
    "section.mobile.bottomSheets.desc": { "es": "La hoja inferior es el contenedor principal de la operación en ruta: muestra la lista de órdenes sobre el mapa y se arrastra entre tres alturas. Incluye el header de ruta, el menú contextual de acciones y las reglas de navegación entre vistas.", "en": "The bottom sheet is the main container of the on-route operation: it shows the order list over the map and drags between three heights. It includes the route header, the contextual action menu and the navigation rules between views." },
    "nav.mobile.toasts": { "es": "Toasts y banners", "en": "Toasts & banners" },
    "section.mobile.toasts.desc": { "es": "Mensajería de feedback de la app móvil. Los toasts oscuros confirman acciones puntuales; la barra de sincronización comunica el estado de conexión; los banners inline (amarillo y azul) entregan contexto persistente dentro de un formulario; el modal de confirmación protege acciones sensibles.", "en": "Feedback messaging of the mobile app. Dark toasts confirm one-off actions; the sync bar communicates connection state; inline banners (amber and blue) provide persistent context inside a form; the confirmation modal protects sensitive actions." },
    "nav.mobile.pod": { "es": "Pruebas de entrega", "en": "Proof of delivery" },
    "section.mobile.pod.desc": { "es": "Componentes para capturar evidencia de entrega y recepción: filas de requisito con estado, captura de foto, firma del receptor, escáneres de código de barras y QR, validación de carga con semáforo por orden, y el formulario de inicio por pasos. Toda evidencia es obligatoria u opcional según la configuración del cliente.", "en": "Components to capture delivery and receipt evidence: requirement rows with state, photo capture, recipient signature, barcode and QR scanners, load validation with per-order traffic light, and the stepped start form. All evidence is mandatory or optional depending on the client configuration." },
    "nav.mobile.orderManagement": { "es": "Gestión de órdenes", "en": "Order management" },
    "section.mobile.orderManagement.desc": { "es": "Componentes del flujo de gestión de una orden: el selector de estado de entrega, el subestado, el acordeón de secciones requeridas, los hitos de operación, la gestión masiva, el recaudo (cash on delivery) y la pantalla de éxito. Es el núcleo operativo de la app.", "en": "Components of the order management flow: the delivery status selector, the substatus, the required-sections accordion, operation milestones, bulk management, cash on delivery and the success screen. It is the operational core of the app." },
    "nav.mobile.maps": { "es": "Mapa", "en": "Map" },
    "section.mobile.maps.desc": { "es": "Elementos del mapa de la app de conductores: pins de parada y de estado, líneas de ruta, controles flotantes (FABs) y el widget de navegación que se superpone sobre Waze o Google Maps. El mapa es la capa base de la operación en ruta, sobre la que se monta el bottom sheet.", "en": "Map elements of the drivers app: stop and status pins, route lines, floating controls (FABs) and the navigation widget that overlays Waze or Google Maps. The map is the base layer of the on-route operation, with the bottom sheet mounted on top." },
    ...
    "nav.mobile.screens": { "es": "Pantallas", "en": "Screens" },
    "nav.mobile.screens.mapList": { "es": "Mapa + Lista", "en": "Map + List" },
    "section.mobile.screens.mapList.desc": { "es": "Plantilla de la pantalla principal de la operación en ruta: mapa a pantalla completa con la lista de órdenes montada sobre un bottom sheet de tres alturas. Es la pantalla donde el conductor pasa la mayor parte de la ruta.", "en": "Template of the main on-route operation screen: a fullscreen map with the order list mounted on a three-height bottom sheet. It is the screen where the driver spends most of the route." },
    "nav.mobile.screens.routeList": { "es": "Lista de rutas", "en": "Route list" },
    "section.mobile.screens.routeList.desc": { "es": "Pantalla de inicio del conductor: lista las rutas asignadas del día y permite declararse disponible para operar. Con el toggle «Disponible» apagado las rutas se muestran deshabilitadas y no se pueden iniciar.", "en": "Driver home screen: it lists the assigned routes of the day and lets the driver declare availability. With the «Available» toggle off, routes are shown disabled and cannot be started." },
    ...
    "nav.emptyState":      { "es": "Estado vacío",             "en": "Empty state" },
    ...
    "section.emptyState.desc":   { "es": "Placeholder centrado para listas, tablas y paneles vacíos. Tile de ícono + título + descripción de una línea, con CTA opcional. Usa CTA primario cuando el usuario puede crear el primer ítem aquí; omítelo cuando los datos se llenan en otra parte.", "en": "Centered placeholder for empty lists, tables and panels. Icon tile + title + one-line description, with an optional CTA. Use a primary CTA when the user can create the first item here; drop it when data fills in elsewhere." },
